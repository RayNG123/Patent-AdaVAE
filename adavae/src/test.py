...
"""
import numpy as np
import torch, math, os, argparse, copy, re
...
def test_short(args):
    ...
    batch_pair1 = {'x': ['<endoftext> a computer input device for provide input control to a computer , the input device comprise : a camera for capture an image of a pattern on a surface ; a processor for determine equation for line from the image of the pattern , apply the line equation to a transform to form transform line equation , and use the transform line equation to determine the height of the computer input device above the surface . <endoftext>',
                        '<endoftext> a method comprising : receive information specify characteristic of a plurality of component link of a composite transport group ; determine an aggregate characteristic of the composite transport group base on the information ; and announce the aggregate characteristic to a label switch network for provision label switch path on the composite transport group . <endoftext>']}
...
if __name__=="__main__":
    ...
    argstr = ('--mode interpolate --add_attn --weighted_sample --latent_size 32 --max_length 20 --batch_size 10 '
